...
""" Given command-line input, formulates and scrapes a URL from bart.gov
detailing the requested trip, including times, connections, and fares.
Uses BeautifulSoup.
"""
...
def determine_time(desired_time):
    ...
    if "now" in desired_time:
        ...
        if hours >= 24 or hours < 0:
            raise ValueError("Invalid time hour input: " + str(hours) +
                             " must be nonnegative and less than 24")
        if minutes >= 60 or minutes < 0:
            raise ValueError("Invalid time minute input: " + str(minutes) +
                             " must be less nonnegative and than 60")
        if minutes != 0:
            if minutes < 15:
                minutes = 0
            elif minutes == 30:
...
def extract_train_direction(tag_line):
    ...
    return tag_string[station_tag_index + 5] + tag_string[station_tag_index+6]\
        + tag_string[station_tag_index+7] + tag_string[station_tag_index+8]
...
def find_all_possible_trips(station1, station2, desired_time="now", query_num=3):
    try:
        ...
        print "query_num must be an integer - it has been set to 3 for this\
               iteration"
        query_num = 3
    ...
    html_doc = urllib2.urlopen("http://bart.gov/schedules/extended.aspx?type=de\
        parture&date=today&time={0}&orig={1}&dest={2}".format(time, orig, dest))
    ...
    for route in routes:
        ...
        if time_compare((hour, minute), chosen_time) == -1:
            ...
            while set_counter < sets:
                print(extract_train_direction(details[set_counter*5 + 2])
                      + "-bound Train:")
                print("\tDEPART: " + extract_time_from_tagline(
                      details[set_counter*5 + 1]) + "\n")
                print("\t\tFROM: " + extract_station_from_tagline(
                      details[set_counter*5]) + "\n")
                print("\tARRIVE: " + extract_time_from_tagline(
                      details[set_counter*5 + 4]) + "\n")
                print("\t\tTO: " + extract_station_from_tagline(
                      details[set_counter*5 + 3]) + "\n")
                set_counter += 1
...
def main(argv):
    try:
        ...
        print '\tpython bart.py departure_station arrival_station time(default\
               = now) number_of_queries(default = 3)'
        sys.exit(2)
    if len(opts) == 2 or len(argv) > 4 and opts[0][0] == '-h':
        print 'usage: \tpython bart.py departure_station arrival_station time\
               (default = now) number_of_queries(default = 3)'
        print '\ndeparture_station and arrival_station names must not include\
               spaces.'
        sys.exit()
    else:
